...
#define ID_SH_TRANSACTION 1001	 //成交(StockStepTrade)
#define ID_SH_ORDERQUEUE 1002	 //委托队列(Stock_OrderQueue_Head+Stock_OrderQueue)
#define ID_SH_STEPORDER 1003	 //逐笔委托(StockStepOrder)
...
#define ID_SH_FASTDEPTHDATA 1012 //上交所撮合行情
#pragma pack(push, 1)
// 1.1 股票行情
...
typedef struct t_SH_StockMarketDataL1
{
	...
	/*  第1位：‘S’表示启动（开市前）时段，‘C’表示集合竞价时段，‘T’表示连续交易时段，‘B’表示休市时段，‘E’表示闭市时段，‘P’表示产品停牌，‘M’表示可恢复交易的熔断时段（盘中集合竞价），‘N’表示不可恢复交易的熔断时段（暂停交易至闭市），‘U’表示收盘集合竞价时段,
	盘后固定价格交易：‘I’表示启动（开市前）时段， ‘A’表示集中撮合时段，‘H’表示连续交易时段，‘D’表示闭市时段，‘F’表示停牌
	第2位： ‘0’表示未连续停牌，‘1’表示连续停牌。无意义填空格。‘0’表示此产品不可正常交易，‘1’表示此产品可正常交易，无意义填空格。
	第3位：‘0’表示未上市，‘1’表示已上市。
	第4位：‘0’表示此产品在当前时段不接受进行新订单申报，‘1’ 表示此产品在当前时段可接受进行新订单申报。无意义填空格。*/
	//深交所：
	//   产品所处的交易阶段代码
...
} T_SH_StockMarketDataL1, *PSH_StockMarketDataL1, StockMarketDataL1;
...
typedef struct t_SH_StockStepTrade
{
	...
	char sRes[3];		 //保留字段1
	T_I64 iBizIndex;	 //业务序号，与委托统一编号，按通道连续
...
typedef struct t_SH_StockOrderQueue
{
	...
	T_OrderQueueItem tItem[2];

} T_SH_StockOrderQueue, *PSH_StockOrderQueue, StockOrderQueue;
...
} T_SH_Kline, *PSH_Kline, T_Kline; // sizeof() = 48
...
typedef struct t_CPXX
{
	...
	char szProdStatusFlag[20];	   //29产品状态标志      C20       该字段为20位字符串，每位表示允许对应的业务，无定义则填空格。	// 如果第4位为‘E’，则表示该产品为沪伦通CDR本地交易的业务产品
...
{
	T_I32 nOrderIndex;	 //委托序号
	T_I32 nOrderChannel; //通道,与成交共用
	T_I32 nOrderTime;	 //委托时间 HHMMSSmmm
	char cOrderType;	 //委托类型 A:增加,D:删除,S:产品状态
	char sRes1[3];		 //保留字段，产品订单状态：ADD-产品未上市、STA-启动、
						 //OCA-开市集合竞价、TRA-连续自动撮合、SUSP-停牌、CLO-闭市、END-交易结束
	T_I64 iOrderNo;		 //原始订单号,类型为产品状态时无意义
	T_I32 nOrderPrice;	 //委托价格，扩大10000倍,类型为产品状态时无意义
	T_I64 iBalance;		 //剩余委托量,类型为产品状态时无意义
	char cOrderBsFlag;	 //买卖标志,B:买单,S:卖单,类型为产品状态时无意义
	char sRes2[3];		 //保留字段
	T_I64 iBizIndex;	 //业务序列号，与成交统一编号，按通道连续

} T_SH_StockStepOrder, *PSH_StockStepOrder; //上交所逐笔委托，对应交易所UA5801
...
//2.0撮合行情
typedef struct t_SH_FastDepthSnapshot
{
	T_I32 nTime;		 //时间(HHMMSSmmmm)
	T_U32 uPreClose;	 //前收盘价,扩大10000倍
	T_U32 uOpen;		 //开盘价,扩大10000倍
	T_U32 uHigh;		 //最高价,扩大10000倍
	T_U32 uLow;			 //最低价,扩大10000倍
	T_U32 uMatch;		 //最新价,扩大10000倍
	T_U32 uAskPrice[10]; //申卖价,扩大10000倍
	T_U32 uAskVol[10];	 //申卖量
	T_U32 uBidPrice[10]; //申买价,扩大10000倍
	T_U32 uBidVol[10];	 //申买量
	T_U32 uNumTrades;	 //成交笔数
	T_I64 iVolume;		 //成交总量
	T_I64 iTurnover;	 //成交总金额
} T_SH_FastDepthSnapshot, *PSH_FastDepthSnapshot;
