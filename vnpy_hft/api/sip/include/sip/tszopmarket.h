...
#define ID_SZOP_BASEINFO 11001   //期权基础信息
#define ID_SZOP_MARKETDATA 11002 //期权快照行情
#pragma pack(push, 1)
//////////////////////////////////////////////////////////////////////////
...
typedef struct t_StockParams
{
	...
	T_U8    uAttribute;// 股票属性 0=普通股票 1=创新企业股票
	char    cNoProfit;//是否尚未盈利  Y=是,未盈利    N=否,已盈利
	char    cWeightedVotingRights;//是否存在投票权差异  Y=存在差异  N=无差异
	char    cIsRegistration;//是否注册制   Y=是   N=否
	char    cIsVIE;//是否具有协议控制架构   Y=是  N=否
} T_STOCKPARAMS;
...
typedef struct t_CashAuctionParams
{
    ...
    struct T_PrcLimitSetsSupply//竞价限价参数补充
	{
		char  cAuctionReferPriceType;//有效范围基准价类型  C1 1=以最近价为参考价  2=以对手方最优价为参考价
	}tPrcLimitSupply[5];
	T_I64   i64MarketBuyQtyUpperLimit;//市价买委托数量的上限  N15(2)
	T_I64   i64MarketSellQtyUpperLimit;//市价卖委托数量的下限 N15(2)
	T_I64   i64MarketBuyQtyUnit;//市价买委托的数量单位  N15(2) 每笔市价买委托的委托数量必须是市价买数量单位的整数倍
	T_I64   i64MarketSellQtyUnit;//市价卖委托的数量单位 N15(2) 每笔市价卖委托的委托数量必须是市价卖数量单位的整数倍
} T_CASHAUCTIONPARAMS;
